...
## En los siguientes data.frames se guarda lo correspondiente al grupo de
## referencia y a la IGR

GR <- subset(AgregCompPunta, RefGrp == refGr)
foc <- subset(GR, INST_BY_REF == InstGR)

## Definicion de si lleva o no Valor Agregado y si se le define o no vecindad.
...
if(is.na(flagVA)){ flagVA <- FALSE}
  ...
  if (flagVA) {
    ...
    comparables <- resto[seq(nParcial), ]
    comparables <- comparables[comparables[, 'dist'] < .3, ]
	TodasComparables <- resto[resto[, 'dist'] < .3, ]
    nComparables <- nrow(comparables)
...
## vecindad
  ...
  if(is.na(preFlag[, 'has15Vec'])){preFlag[, 'has15Vec'] <- FALSE}
  if(preFlag[, 'has15Vec']) {
...
otros indicadores de calidad de las Instituciones de Educaci\'on
Superior 2013-2014\vspace{62mm}}
\end{center}
}
...
if(flag2){
Sweave('ResPRO2013_2014v01.rnw', encoding = "latin1")
    if (flagVA & nComparables > 0) {
      Sweave('AR2013_2014v01.rnw', encoding = "latin1")
    }
    Sweave('OCalidad2013_2014v01.rnw', encoding = "latin1")
...
if(flag2){
    cat('\\input{ResPRO2013_2014v01}\n')
    if (flagVA & nComparables > 0) {
      cat('\\input{AR2013_2014v01}\n')
    }
    cat('\\input{OCalidad2013_2014v01}\n')
    if (flagVec & nComparables > 0) {
